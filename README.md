...
- `-m`, `--mode MODE`
  ...
  - `8`: MZ-2500標準16色のうち、従来の8bitパソコンで使われていた8色のみ使用して減色
  - `16`: MZ-2500標準16色で減色
...
- `--layout MODE`
  - 指定値: `640x400`,`640x200`,`320x200`,`split320x200`
  - MZ-2500で画面全体に表示するピクセル数を指定する。
...
- `-d`, `--method METHOD`
  - 指定値: `floyd_steinberg`, `stucki`, `jarvis`, `no_dither`
  - 誤差拡散手法の指定をする。
...
- `-s`, `--sort MODE`
  - 指定値: `no_sort`, `luminance`, `frequency` 
...
- `--d88` を使用すると、生成した `BRD` / `BSD` をD88ディスクイメージへ追加できます。
